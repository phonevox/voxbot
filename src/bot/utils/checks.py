...
def is_me():
    def predicate(interaction: discord.Interaction) -> bool:
        bot_owner = int(
            os.getenv("BOT_OWNER", "0")
        )  # Converte para int com fallback para 0
...
def is_test_guild():
    def predicate(interaction: discord.Interaction) -> bool:
        return interaction.guild.id == os.getenv(
            "TESTING_GUILD_ID" or interaction.guild.id in os.getenv("TESTING_GUILD_ID")
        )
